 ...
 * We primarily use it to store (and modify) attributes on VectorObject structures.
 * It's analogous to the NSDictionary class on iOS, but simpler.
 */
public class AttrDictionary
{
	 ...
	 */
	public native @Nullable String getString(@NotNull String attrName);
	 ...
	 */
	public native @Nullable Integer getInt(@NotNull String attrName);
	 ...
	 */
	public native @Nullable Double getDouble(@NotNull String attrName);
	 ...
	 */
	public native @Nullable Long getIdentity(@NotNull String attrName);

	/**
	 * Convert the given attribute to a boolean or return false if it's not there.
	 */
	public boolean getBoolean(@NotNull String attrName) {
		final AttrDictionaryEntry entry = getEntry(attrName);
		if (entry == null) {
			return false;
		}
		switch (entry.getType()) {
			case DictTypeDouble:   return entry.getDouble() != 0.0;
			case DictTypeInt:      return entry.getInt() != 0;
			case DictTypeIdentity: return entry.getIdentity() != 0;
			case DictTypeString:
				final String val = entry.getString();
				return val != null && (
	 ...
	 */
	public native @Nullable Object get(String attrName);
	 ...
	 */
	public native @Nullable AttrDictionary getDict(String attrName);

	/**
	 * Return a generic dictionary entry for the given attribute.
	 * Returns null if there wasn't one.
	 */
	public native @Nullable AttrDictionaryEntry getEntry(String attrName);
	 ...
	 */
	public native @Nullable AttrDictionaryEntry[] getArray(String attrName);

	/**
	 * Return all the top level attribute names.
	 */
	public native @Nullable String[] getKeys();
