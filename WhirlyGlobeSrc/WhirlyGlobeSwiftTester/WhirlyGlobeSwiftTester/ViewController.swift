...
class ViewController: UIViewController {
	...
	override func viewDidLoad() {
		...
		if doGlobe {
			...
			mapViewC = MaplyViewController(mapType: MaplyMapType.TypeFlat)
			theViewC = mapViewC
		}
		...
		if let globeViewC = globeViewC {
			...
			globeViewC.animateToPosition(MaplyCoordinateMakeWithDegrees(Float(-122.4192), Float(37.7793)), time: 1.0)
		}
		else if let mapViewC = mapViewC {
			mapViewC.height = 1.0
			mapViewC.animateToPosition(MaplyCoordinateMakeWithDegrees(-Float(122.4192), Float(37.7793)), time: 1.0)
		}
