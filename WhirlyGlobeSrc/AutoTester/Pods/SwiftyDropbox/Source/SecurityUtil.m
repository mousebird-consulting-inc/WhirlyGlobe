...
@implementation SecurityUtil : NSObject
...
+ (CFDataRef) copySerialFromCertificate:(const uint8_t *)derdata certificateLength:(const CFIndex)certlen {
    ...
    uint32_t len = 0;
    if ([SecurityUtil readLength:&len fromBuffer:derdata ofLength:derlen atIndex:&idx] != 0) {
        return NULL;
    }
...
+ (bool)isRevokedCertificate:(SecCertificateRef)certificate {
    ...
    const CFDataRef serial = [SecurityUtil copySerialFromCertificate:CFDataGetBytePtr(cfder) certificateLength:CFDataGetLength(cfder)];
    CFRelease(cfder);
...
+ (NSArray *)rootCertificates {
    ...
    @synchronized ([SecurityUtil class]) {
        if (sRootCerts == NULL) {
            sRootCerts = [NSMutableArray array];
            for (int i=0; i < sizeof(kBase64RootCerts)/sizeof(kBase64RootCerts[0]); i++) {
                ...
                NSData *base64Cert = [[NSData alloc] initWithBytes:kBase64RootCerts[i] length:base64CertLen];
                NSData *derCert = [[NSData alloc] initWithBase64EncodedData: base64Cert options:0];
                ...
                [sRootCerts addObject:(id)CFBridgingRelease(cert)];
            }
        }
    }
    return sRootCerts;
